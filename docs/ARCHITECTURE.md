...
취업 지원 자동화 AI 에이전트. 채용공고를 넣으면 자소서 초안이 나오고,
지원 이력이 관리되며, 마감이 다가오면 메일로 알려준다.
...
채용공고 텍스트
     │
     ├─→ reader.py ──── 파일 형식별로 텍스트 추출 (txt/pdf)
     │        │
     │        ↓
     ├─→ jd_parser.py ── LLM으로 구조화 → JSON ────→ db.py ──→ SQLite
     │        │                                        │
     │        ↓                                        │
     ├─→ draft.py ───── ① 경험 선별                    │
     │        │         ② 자소서 작성  ──→ .md 파일    │
...
| 경로 | 내용 | 중요도 |
|---|---|---|
| `data/experiences.json` | **내 경험 원자 DB.** 자소서의 유일한 재료 | ★★★ |
| `data/jarvis.db` | 지원 이력 (SQLite) | ★★★ |
| `data/jobs/*.txt` | 공고 원문 | ★ |
| `data/jobs/*.json` | 파싱된 공고 | ★ |
| `data/drafts/*.md` | 생성된 자소서 초안 | ★★ |
| `data/drafts/*_meta.json` | 선별 근거 + gaps | ★★ |
| `data/notes.md` | 실험 기록 | ★★ |
| `data/scheduler.log` | 자동 실행 로그 | ★ |
| `.env` | API 키, 메일 비밀번호 | ★★★ (비공개) |
...
### 5-6. 왜 수집 계층과 처리 계층을 분리했나

`reader.py`가 파일 형식을 판단하고, `jd_parser.py`는 텍스트만 받는다.
...
나중에 워크넷 API나 크롤링을 붙여도 **파서는 안 건드려도 된다.**
분기 하나만 추가하면 끝.

### 5-7. 왜 메일 발송은 try/except로 감쌌나
...
### 5-8. 왜 APScheduler 대신 Windows 작업 스케줄러인가
...
## 6. 데이터 흐름 상세 (공고 1건 처리 시)

```
1. 사용자가 data/jobs/xxx.txt 작성
      ↓
2. python -m src.jd_parser data/jobs/xxx.txt
      ↓
   ...
   db.upsert() + db.set_status('parsed')
      ↓
3. python -m src.draft data/jobs/xxx.json
      ↓
   ...
   notify.notify_draft() → mailer.send() → 메일 발송
      ↓
4. [사람] 초안 검토·수정 후 실제 지원
      ↓
5. 메일 버튼 클릭 → server.action() → db.set_status('applied')
   또는 python -m src.cli set 1 applied
      ↓
6. 매일 9시: 작업 스케줄러 → run_daily.bat → daily.run_daily()
...
| 대시보드 화면 | `app.py` |
| 공고 자동 수집 | 새 모듈 → `reader` 또는 `jd_parser` 로 연결 |
...
| 기능 | 왜 미뤘나 |
|---|---|
| Google Calendar 연동 | OAuth 절차가 번거롭고, 등록할 일정이 아직 적다 |
| 워크넷 API 공고 수집 | 지원 건수가 쌓인 뒤에 가치가 생긴다 |
